...
@section('title', __('reports.moneyreport') )
...
<div class="dulni">
  ...
  <div class="  px-0 banner" id="dulani-banner">
    <div class="dulni-inner">
      <div class="container">
        <div class="page-path">
          <p><a href="{{url("/")}}"> الرئيسية </a>/ {{__('reports.moneyreport')}} </p>
        </div>
      </div>
    </div>
      ...
      <!-- Start Dullani Form-->
        <div class="allfiles mx-auto">
          <div class="container" id="dulani-form-container">
            ...
            <h3 class="text-center mt-5 mb-2">  {{ __('reports.moneyreport') }} </h3>
...
{{-- //  `ReportId`, `reportTitle`, `reportImageFile`, `reportPdfFile`, `reportStatus`, `created_at`, `updated_at` --}}
                ...
                @forelse ($allreportFiles as $file)
                  <div class="col-sm-12 col-md-4 4 mt-5 center-phone">
                    <a target="_blank"
                    class="d-block main-color"
                    href="{{url($file->reportPdfFile)}}">
                    <img style="width:100px" src="{{url($file->reportImageFile)}}" alt="{{ $file->reportTitle }}">
